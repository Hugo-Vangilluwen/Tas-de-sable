...
module Floats : EltsI.ORDERED_AND_OPERATIONAL with type t = float =
struct
  ...
  let from_string (x: string) =
    try
      float_of_string x
    with
    | Failure (message) when message = "float_of_string" -> raise NonElt
...
(* Commented from the source because we don't use Num library *)
(*
